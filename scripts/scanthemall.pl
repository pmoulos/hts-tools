...
--- Optional ---
  ...
  --times|n     How many times should the background set of sequences 
            ...
            score from --range, be larger than the input set? It 
  ...
  --justscan|j      Set this to a real number between 0 and 1 (preferably 
            close to 0.9), to just scan the input sequences using this 
            score cutoff for the motifs without defining an FPR. Useful 
            if you have a very limited number of input sequences (e.g. 
            just one).
