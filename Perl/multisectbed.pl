...
sub create_venn
{
    ...
    switch(scalar @input)
    {
        ...
        case 5 {
            ...
            "\t)\n".
            "grid.p <- apply(as.matrix(expand.grid(rep(list(c(\"-\",\"+\")),5))),1,function(x) paste(x,collapse=\"\"))\n".
            "grid.p <- grid.p[2:length(grid.p)]\n".
            "grid.n <- numeric(length(grid.p))\n".
            "map <- c(1,2,9,3,12,11,22,4,8,14,19,13,23,24,30,5,7,10,20,6,17,21,29,15,18,25,28,16,27,26,31) + 10\n".
            "names(grid.n) <- names(map) <- grid.p\n".
            "for (n in grid.p) {\n".    
            "\tgrid.n[n] <- as.numeric(v[[map[n]]]\$label)\n".
            "}\n".
